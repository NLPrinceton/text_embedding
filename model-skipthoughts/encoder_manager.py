...
import importlib
configuration = importlib.import_module('model-skipthoughts.configuration')
skip_thoughts_encoder = importlib.import_module('model-skipthoughts.skip_thoughts_encoder')

#-----------------------------------------------------------------------------#
# Specify model location here
#-----------------------------------------------------------------------------#
path_to_models = '/n/fs/nlpdatasets/SkipThoughts/'
#-----------------------------------------------------------------------------#

path_to_umodel = path_to_models + 'skip_thoughts_uni_2017_02_02/'
path_to_bmodel = path_to_models + 'skip_thoughts_bi_2017_02_16/'
skipthoughts= {'uni': {'vocabulary_file': path_to_umodel+'vocab.txt',
                        'embedding_matrix_file': path_to_umodel+'embeddings.npy',
                        'checkpoint_path': path_to_umodel+'model.ckpt-501424'},
                'bi': {'vocabulary_file': path_to_bmodel+'vocab.txt',
                       'embedding_matrix_file': path_to_bmodel+'embeddings.npy',
                       'checkpoint_path': path_to_bmodel+'model.ckpt-500008'}}
...
class EncoderManager(object):
  ...
  def load_uni(self):
    self.load_model(configuration.model_config(bidirectional_encoder=False), **skipthoughts['uni'])

  def load_bi(self):
    self.load_model(configuration.model_config(bidirectional_encoder=True), **skipthoughts['bi'])

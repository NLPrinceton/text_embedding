...
import importlib
skip_thoughts_model = importlib.import_module('model-skipthoughts.skip_thoughts_model')
special_words = importlib.import_module('model-skipthoughts.data.special_words')
...
class SkipThoughtsEncoder(object):
  ...
  def encode(self,
    ...
    if use_norm:
      thought_vectors = [v/normv if normv else v for v, normv in ((v, norm(v)) for v in thought_vectors)]

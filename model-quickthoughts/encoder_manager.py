...
import importlib
s2v_encoder = importlib.import_module('model-quickthoughts.s2v_encoder')
...
tf.flags.DEFINE_integer("sequence_length", 1000, "Max sentence length considered")
tf.flags.DEFINE_float("uniform_init_scale", 0.1, "Random init scale")
tf.flags.DEFINE_string("results_path", "/n/fs/nlpdatasets/QuickThoughts", "Model results path")
tf.flags.DEFINE_string("Glove_path", "/n/fs/nlpdatasets/QuickThoughts", "Path to Glove dictionary")
#with open('text_embedding/model-quickthoughts/eval.json', 'r') as f:
#    config = json.load(f)
#CONFIG = configuration.model_config(config, mode="encode")
...
#    with open('text_embedding/model-quickthoughts/'+model_config+'.json', 'r') as f:
#        config = json.load(f)

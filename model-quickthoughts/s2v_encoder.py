...
import importlib
s2v_model = importlib.import_module('model-quickthoughts.s2v_model')
...
class s2v_encoder(object):
  ...
  def encode(self,
    ...
    for batch, start_index in enumerate(batch_indices):
      ...
      if use_norm:
        enc = [v/normv if normv else v for v, normv in ((v, norm(v)) for v in enc)]
        enc_out = [v/normv if normv else v for v, normv in ((v, norm(v)) for v in enc_out)]
